		...
		if centosversion 6; then
			...
			if [ $? -eq 0 ]; then
				...
				if [ $? -ne 0 ]; then
					...
					iptables -I INPUT -m state --state NEW -m udp -p udp --dport 19999 -j ACCEPT
			...
			if [ $? -eq 0 ];then
				...
				if [ $? -eq 0 ];then
					...
					if [ $? -eq 0 ];then
						...
						echo "WARNING: Try to start firewalld failed. please enable port 22/80/443/19999 manually if necessary."
					fi
				fi
			fi
		fi
...
set_netdata(){
 ...
 echo "更多配置请查看官网文档：https://github.com/firehol/netdata"
 echo "--------------------"
}
...
checkos                 #判断操作系统
firewall_set            #配置防火墙开放19999端口
disable_selinux         #设置系统禁用selinux
